...
def buffer_operation(data: BufferRequest):
    try:
        gis.buffer(distance=data.distance, feature_id=data.feature_id)
        return {"status": "success", "operation": "buffer", "distance": data.distance, "feature_id": data.feature_id}
    except Exception as e:
...
def intersect_operation(data: GeometryRequest):
    intersected = gis.intersect(data.geometry)
    return {"status": "success", "operation": "intersect", "count": len(intersected), "features": json.loads(intersected.to_json())}
...
def nearest_operation(data: GeometryRequest):
    ...
    return {"status": "success", "nearest": result}
